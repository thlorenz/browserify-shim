...
var path             = require('path')
  ...
  , fixPackages      = require('./fix-packages')
...
var resolve = module.exports = function resolveShims (file, messages, cb) {
  ...
  mothership(file, function (pack) { return !! pack['browserify-shim'] }, function (err, res) {
    ...
    if (!res.pack) return cb(new Error('That module is outside of the tree, what is going on here?'));
    ...
    // only get here if we haven't seen this package.json file before
    fixPackages(packageDir, pack, messages, function (err) {
      if (err) return cb(err);
