...
function resolvePaths (packageDir, shimFileDir, redirects, shims) {
  return Object.keys(shims)
    .reduce(function (acc, relPath) {
      var shim = shims[relPath];
      var redirect = redirects[relPath];
      var shimPath;

      if (redirect) {
        // redirect (alias) specified for this lib in package.json's browser field
        ...
        shimPath = path.resolve(packageDir, redirect);
      } else if (shimFileDir) {
...
if (!module.parent && typeof window === 'undefined') {
  ...
  resolve(require.resolve('../test/nodeps/extshim-redirect/vendor/non-cjs'), function (err, res) {
